 ...
 * Based on the rules provided by Dr. Khalid M. Al-Kahsah.
...
program
    : PROGRAM ID SEMICOLON block DOT; // بداية البرنامج

block
    : definitions_part? instructions_list;
...
constant_def
    : ID EQUALS constant_value SEMICOLON;

types_definition
    : TYPE (type_def)+;

type_def
    : ID EQUALS composite_type SEMICOLON;

variables_definition
    : VARIABLE (variable_def)+;

variable_def
    : variables_group SEMICOLON;
...
record_type
    : RECORD L_CURLY_BRACE fields_list R_CURLY_BRACE;

fields_list
    : field_def (SEMICOLON field_def)*;

field_def
    : ID (COMMA ID)* COLON data_type;
...
param_def
    : (BY_VALUE | BY_REFERENCE)? variables_group;
...
assignment_statement
    : variable_access EQUALS expression;

input_statement
    : READ L_PAREN variable_access R_PAREN;

output_statement
    : PRINT L_PAREN print_list R_PAREN;

call_statement
    : ID L_PAREN actual_params_list? R_PAREN;

print_list
    : print_item (COMMA print_item)*;

print_item
    : variable_access | literal_value;
...
selector
    : indexed_selector | field_selector;
...
constant_value
    : numeric_value | literal_value | logical_value | ID;
...
relational_op
    : GT | LT | GTE | LTE | EQUALS_OP | NOT_EQUALS_OP;
...
ID              : ARABIC_LETTER (ARABIC_LETTER | DIGIT)*;
